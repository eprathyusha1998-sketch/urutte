...
# Script to link existing AI posts to topics (fixed version)
# This will create ai_generated_threads records for existing AI posts

echo "🔗 Linking AI posts to topics (fixed version)..."
...
-- Check the structure of ai_generated_threads table
\echo 'Checking ai_generated_threads table structure:'
\d ai_generated_threads
...
-- Create ai_generated_threads records for existing AI posts (using correct column names)
...
-- Link posts to topics based on content (using correct column names)
INSERT INTO ai_generated_threads (thread_id, topic_id, ai_admin_id, created_at, updated_at)
SELECT 
    t.thread_id,
    CASE 
